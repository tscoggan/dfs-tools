...
trait DFSScoringSystem {
  def calculateFantasyPoints(player: PlayerGameStats): Float
}

object FanDuelMLB extends DFSScoringSystem {
  override def toString: String = "FanDuel MLB"

  def calculateFantasyPoints(player: PlayerGameStats): Float = player match {
    case hitter: HitterGameStats =>
