...
case class HistoricalStats(season: Season) {
  ...
  case class HitterStats(p: Player, opposingPitcher: Player) {
    ...
    val valueScoreFD: Option[Double] = projValueFD.map(value => (projFptsFD.getOrElse(0.0) * Configs.stackValueScoreFptsMultiplier) + value)
    ...
    val valueScoreDK: Option[Double] = projValueDK.map(value => (projFptsDK.getOrElse(0.0) * Configs.stackValueScoreFptsMultiplier) + value)
